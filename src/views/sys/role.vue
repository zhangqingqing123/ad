<template>
  <div class="app-container">
    <div class="filter-container">
      <el-form>
        <el-form-item>
          <el-button type="primary" class="filter-item" icon="el-icon-edit" @click="showCreate">添加角色
          </el-button>
        </el-form-item>
      </el-form>
    </div>
      ...
      <el-table-column align="center" label="管理" width="220">
        <template slot-scope="scope">
          <div v-if="scope.row.roleName!='管理员'">
            ...
            <el-button type="danger" icon="delete" @click="removeData(scope.$index)"> 删除
            </el-button>
          </div>
        </template>
      </el-table-column>
...
<script>
import {getPermisson,getListRole,addRole,updateRole,removeRole} from '@/api/role'
  ...
  export default {
    ...
    methods: {
      getAllPermisson() {
        getPermisson().then(data => {
          this.allPermission = data.data.data;
        })
      },
      getList() {
	      this.listLoading = true
	      getListRole(this.listQuery).then(response => {
      ...
      removeData($index) {
        ...
        let role = _vue.list[$index];
	    	removeRole(role).then(response => {
		        ...
		        const index = this.list.indexOf(role)
