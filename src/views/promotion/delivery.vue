		...
		<el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit highlight-current-row>
					...
					<el-form label-position="left" class="table-expand">
						<div v-for="w in props.row.goodsList">
							...
							<el-form-item label="商品类型：">
								<span>{{w.collectItem.cateName}}&nbsp;&nbsp;&nbsp;&nbsp;X{{w.num}}</span>
							</el-form-item>
							<el-form-item label="商品分类：">
								<span>{{w.collectItem.cateTypeName}}</span>
							</el-form-item>
							<el-form-item label="商品规格：">
								<div>
									<span v-for="v in w.collectItem.skus">{{ v.specName }}&nbsp;&nbsp; {{ v.inventory }}库存、</span>
								</div>
			...
			<el-table-column align="center" label="发货状态" prop="couponIntegral">
				<template slot-scope="scope">
					...
					<span v-else>{{scope.row.status== 4?'已完成':'其他'}}</span>
				</template>
			</el-table-column>
			...
			<!--<el-table-column align="center" label="合伙人抽成" prop="referrerPartnerCount">
			</el-table-column>
			
			<el-table-column align="center" label="店员积分" prop="referrerClerkCount">
			</el-table-column>
