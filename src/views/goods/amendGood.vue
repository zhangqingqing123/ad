	...
	<div class="app-container calendar-list-container">
		...
		<el-card class="box-card">
			...
			<el-form :rules="rules" ref="goods" :model="goods" label-width="150px">
				...
				<!--<el-form-item label="商户地址" prop="selectedOptions">
					<el-cascader :options="cateAreaList"  v-model="goods.selectedOptions"  @change="handleCategoryChange">
					</el-cascader>
				</el-form-item>-->
				<el-form-item label="商品图片">
					<!--<el-upload class="avatar-uploader" :action='uploadPath' list-type='picture-card' :show-file-list="false" accept=".jpg,.jpeg,.png" :on-success="uploadPicUrl" :before-upload="beforeAvatarUpload">
				...
				<el-form-item label="宣传画廊">
					<!--<el-upload :action='uploadPath' :limit='3' multiple accept=".jpg,.jpeg,.png" :file-list="briefImg" list-type="picture-card" :on-exceed="uploadOverrun" :on-success="handleGalleryUrl" :on-remove="handleRemove" :before-upload="beforeAvatarUpload">
		...
		<el-card class="box-card">
			...
			<el-table :data="skus">
				...
				<!--<el-table-column property="specificationValue" label="规格值"></el-table-column>-->
				...
				<el-table-column align="center" label="操作" width="250" class-name="small-padding fixed-width" v-if="multipleSpec">
					<template slot-scope="scope">
						...
						<el-button type="danger" size="mini" @click="handleSpecificationAdmin(scope.row)">修改</el-button>
					</template>
				</el-table-column>
			</el-table>
			<!--规格值设定-->
			<el-dialog title="设置规格" :visible.sync="specVisiable">
				...
				<div slot="footer" class="dialog-footer">
					<el-button type="primary" @click="handleSpecificationAdd" v-if="specIndex">确定</el-button>
					<el-button @click="specVisiable = false" v-else>确定</el-button>
				</div>
			</el-dialog>
		</el-card>
...
<script>
	import { addListGoods, addItemGoods, detailItemGoods, gitCategory, getTypes, listArea, getGoodsDetail, getItems, getRole } from '@/api/goods'
	...
	export default {
		...
		methods: {
			...
			//得到商品类目
			getTypes() {
				getTypes(this.dataForm).then(response => {
			...
			handleCategoryChange(value) { //省市区改变的函数；
			...
			getupload1: function(event) {
				...
				$(event.srcElement).uploadHeader({
					...
					ok: function(dataUrl) {
						...
						var file = new File([u8arr], 'ght_image', {
							type: mime
						});
...
//			删除上传
			...
			handlePublish: function() {
				...
				this.$refs['goods'].validate((valid) => {
					if(valid) {
						...
						//this.skus[0].specName = specHtml; //获取规格；
						//console.log(this.products);
						//this.skus[0].inventory = this.products[0].number; //获取规格；
						//						const finalGoods = {
						...
						//						}
						_this.finalGoods.cateId = _this.goods.cateId; //商品类型
						_this.finalGoods.description = _this.description; //商品描述，富文本内容， 可图文
						_this.finalGoods.id = _this.goods.id, //商品ID
							_this.finalGoods.pictures = _this.pictures,
							_this.finalGoods.shopId = _this.goods.shopId, //商户ID
							_this.finalGoods.skus = _this.skus;
						_this.finalGoods.subCateId = _this.goods.subCateId, //商品类目
							_this.finalGoods.title = _this.goods.title, //标题
							_this.finalGoods.goods = _this.goods, //商品
							_this.finalGoods.specifications = _this.specifications, //规格
							_this.finalGoods.products = _this.products, // 产品
							_this.finalGoods.attributes = _this.attributes //属性
			...
			getGoodsDetail(id) {
				...
				getGoodsDetail(id).then(response => {
					...
					var specificationsArry = response.data.data.skus[0].specName;
					console.log(specificationsArry)
					var newSpecifications = '';
					var specifications = [];
					var getspecifications = [];
					if(specificationsArry) {
						newSpecifications = specificationsArry.substring(0, specificationsArry.length - 1).split(',');
						for(var i = 0; i < newSpecifications.length; i++) {
							specifications.push(newSpecifications[i].split('-'));
						}
						for(var i = 0; i < specifications.length; i++) {
							getspecifications.push({
								"specification": specifications[i][0],
								"value": specifications[i][1],
								"picUrl": ''
					...
					this.getTypes();
					this.gitCategory();
					//					this.goods.selectedOptions 	= [response.data.data.provinceId, response.data.data.cityId, response.data.data.districtId];
					this.goods.cateType = 1; //种类
					this.goods.cateId = response.data.data.cateId; //类型
					this.goods.subCateId = response.data.data.subCateId; //类目
					this.goods.shopId = response.data.data.shopId; //类目
					this.goods.target = response.data.data.shopName;
					var res = response.data.data;
					this.goods.cateId = res.cateId //商品类型
					this.goods.cateType = 1 //商品类别
					//					this.goods.city       = res.city.id //市
					this.description = res.description //商品描述，富文本内容， 可图文
					//					this.goods.district   = res.district.id //区
					this.goods.id = res.id //商品ID
					this.pictures = res.pictures;
					//console.log(res.pictures);
					var picturesArry = res.pictures;
					var sysConfig = this.$store.getters.sysConfig.storageBaseUrl;
					//console.log(picturesArry);
					var n = '';
					console.log(this.sysConfig);
					for(var i = 1; i < picturesArry.length; i++) {
						this.briefImg.push({
							url: sysConfig + '/' + picturesArry[i].itemPicUrl,
							itemPicUrl: picturesArry[i].itemPicUrl,
							itemPicKey: picturesArry.itemPicUrl,
							picType: 2
						})
					}
					this.sysConfig = sysConfig;
					//this.province=res.province.id //省
					this.shopId = res.shopId //商户ID
					this.skus = res.skus
					this.subCateId = res.subCateId //商品类目
					this.goods.title = res.title //标题
					this.products[0].number = response.data.data.skus[0].inventory;
				})
			},
			handleClose(tag) {
			...
			handleRemove: function(file, fileList) {
				...
				console.log(file);
				for(var i = 1; i < this.pictures.length; i++) {
					// 这里存在两种情况
					...
					if(this.$store.getters.sysConfig.storageBaseUrl + '/' + this.pictures[i].itemPicUrl === url) {
			...
			handleSpecificationAdmin(row) {
				this.myskus = row
				this.specIndex = false
				this.specVisiable = true
				this.$nextTick(() => {
					this.$refs['myskus'].clearValidate()
				})
