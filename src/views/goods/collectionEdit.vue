<template>
  ...
  <!--<el-card class="box-card">-->
    ...
    <el-form :rules="rules" ref="goods" :model="goods" label-width="150px">
      ...
      <el-card class="box-card">
      	<h3>藏品信息</h3>
      	<el-form-item label="证书模板" prop="certTempId">
        	<el-select v-model="goods.certTempId" placeholder="请选择">
				    <el-option
      ...
      <el-card class="box-card">
          ...
          <el-input v-model="goods.originalPrice">
      ...
      <el-card class="box-card">  
        ...
        <el-form-item label="简介图">
          <!--<el-upload :action='uploadPath' :limit='5' multiple accept=".jpg,.jpeg,.png,.gif" :file-list="briefImg" list-type="picture-card" :on-exceed="uploadOverrun" :on-success="handleGalleryUrl" :on-remove="handleRemove">
        ...
        <el-form-item label="检验机构证书上传">
          <el-upload :action='uploadPath' :limit='5' multiple accept=".jpg,.jpeg,.png,.gif" :file-list="certificateImg" list-type="picture-card" :on-exceed="uploadOverrun" :on-success="handleCertificateUrl" :on-remove="handleCertificateRemove">
             <i class="el-icon-plus"></i>
          </el-upload>
          <!--<div style="display: flex;">
...
import { templateCollect, collectionCreate, detailItemCollection, categoryListCollection} from '@/api/collection'
...
//				}]
  ...
  methods: {     
    init: function() {
      ...
      detailItemCollection(id).then(response => {
//			console.log(response)
        	this.goods = response.data.data
//      	this.templateValue = response.data.data
...
//	        		})
							this.briefImg.push({
								url:this.sysConfig+this.goods.pictures[index].itemPicUrl,
								itemPicUrl:this.goods.pictures[index].itemPicUrl,
								itemPicKey:this.sysConfig+this.goods.pictures[index].itemPicUrl,
								picType:2
							})
						} else if(this.goods.pictures[index].picType == 1){
							this.outlayImg[0].itemPicUrl = this.goods.pictures[index].itemPicUrl
							this.outlayImg[0].itemPicKey = this.sysConfig+this.goods.pictures[index].itemPicUrl
						} else if (this.goods.pictures[index].picType == 4){
//							this.galleryFileList.push({
//	        			url:this.storageBaseUrl+'/'+this.goods.pictures[index].itemPicUrl
//	        		})
...
//									  	_this.speedProgress = res.total.percent
	...
	//								  成功
			...
			getupload3:function(event){
				console.log(this.outlayImg.length)
				if(this.outlayImg.length > 4){
					alert("最多上传3张")
					return
				} else {
					let _this = this
...
//				certTempId:this.goods.certTempId,
...
//			}
				...
				_this.finalFoods.display 				= _this.goods.display,			
				_this.finalFoods.collectDesc 		= _this.goods.collectDesc,
				_this.finalFoods.pictures 			= _this.certificateImg.concat(_this.outlayImg.concat(_this.briefImg))			
